...
const card = (b) =>
  `<article class="book-item" data-world="${b.world}"><a class="book-object" href="/books/${b.slug}/">${cover(b)}</a><p class="eyebrow">${esc(b.series)} · ${b.number}</p><h3><a href="/books/${b.slug}/">${esc(b.title)}</a></h3><a class="text-link" href="/books/${b.slug}/">Discover the story <span aria-hidden="true">↗</span></a></article>`;
...
for (const b of books) {
  const related = books.filter(
    (x) => x.series === b.series && x.slug !== b.slug,
  );
  await page(
    "/books/" + b.slug + "/",
    layout(
      b.title,
      `<main id="main" class="book-page"><div class="breadcrumb"><a href="/#books">← Back to the library</a></div><section class="book-detail section ${b.world}"><div class="detail-cover">${cover(b)}${!b.cover ? '<p class="art-note">Temporary typographic cover · final artwork to come</p>' : ""}</div><div><p class="eyebrow">${esc(b.series)} · BOOK ${b.number}</p><h1>${esc(b.title)}</h1><p class="genre">${esc(b.genre)}</p><p class="synopsis">${esc(b.description)}</p>${external(amazon(b), b.amazon ? "See the book on Amazon" : "Find this title on Amazon", "button")}<p class="purchase-note">${b.amazon ? "View available editions and purchase on Amazon." : "Opens an Amazon search for this title and W.T. Brooks."}</p><details><summary>Choosing a book for a reader?</summary><p>Check the Amazon listing for the current description, reading age, available formats, and any sample supplied by the author.</p></details></div></section><section class="section"><p class="eyebrow">KEEP EXPLORING</p><h2>More from ${esc(b.series)}</h2><div class="book-grid related">${related.map(card).join("")}</div></section></main>`,
      "/books/" + b.slug + "/",
      b.description,
      {
        "@type": "Book",
